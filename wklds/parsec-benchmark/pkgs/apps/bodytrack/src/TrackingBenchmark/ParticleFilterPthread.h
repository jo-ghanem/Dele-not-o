...
class ParticleFilterPthread: public ParticleFilter<T>, public threads::Threadable {
	...
	using ParticleFilter<T>:: this->mModel;
	...
	if (cmd == workers.THREADS_CMD_PARTICLEWEIGHTS) {
		...
		while(ticket < mNParticles) {
			...
			for(i = ticket; i < mNParticles && i < ticket + WORKUNIT_SIZE_NEWPARTICLES; i++) {
				...
				this->AddGaussianNoise(mNewParticles[i], this->mModel->StdDevs()[annealing_parameter], mRnd[i]);
			}
			ticket = particleTickets.getTicket();
		}
	} else {
...
{	bool valid;
	mWeights[i] = this->mModel->LogLikelihood(particles[i], valid, rank);                   //compute log-likelihood weights for each particle
	return valid;
}

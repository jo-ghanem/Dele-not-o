...
void Frame()
{
  ...
  Global = (struct GlobalMemory *)({ void* mem = malloc(sizeof(struct GlobalMemory)); assert(mem); mem; });;
...
*/
  my_node = ({; uint64_t ___x =atomic_fetch_add(&(Global->Index), 1); ; ___x;});
...
;
    ...
    for(i = 0; i < num_nodes; ++i) {
      printf("%3ld\t%3ld\t%6ld\t%6ld\t%6ld\t%6ld\t%8ld\n",i,
            Global->progress[i].frame,
            Global->progress[i].exectime,
	          Global->progress[i].exectime1,
            Global->progress[i].num_rays_traced,
            Global->progress[i].num_traced_rays_hit_volume,
	          Global->progress[i].num_samples_trilirped);
    }
...
void Allocate_Image(PIXEL **address, long length)
{
  ...
  *address = (PIXEL *)({ void* mem = malloc(length*sizeof(PIXEL)); assert(mem); mem; });;
...
void Allocate_MImage(MPIXEL **address, long length)
{
  ...
  *address = (MPIXEL *)({ void* mem = malloc(length*sizeof(MPIXEL)); assert(mem); mem; });;
...
/*  POSSIBLE ENHANCEMENT:  If you want to replicate the shade table,
  ...
  *address1 = (PIXEL *)({ void* mem = malloc(length); assert(mem); mem; });;

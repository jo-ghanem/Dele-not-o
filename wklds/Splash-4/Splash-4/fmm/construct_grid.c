...
MLGHelper (long my_id, box *local_box, box *global_box, box *global_parent)
{
   ...
   while (success == FALSE) {
	 ...
	 /*
		...
		*/
		int gbt = atomic_load(&(global_box->type));
	    if (gbt == PARENT) {
	       success = TRUE;
...
#endif // WITH_NO_OPTIONAL_LOCKS
*/
				 box * gbc = atomic_load(&(global_box->children[i]));
...
#endif // WITH_NO_OPTIONAL_LOCKS
*/
	    global_box = atomic_load(&(global_parent->children[local_box->child_num]));
     }
      }
   }
}
...
#endif // WITH_NO_OPTIONAL_LOCKS
*/
	 child = atomic_load(&(pb->children[i]));
	 if (child == NULL) {
...
#endif // WITH_NO_OPTIONAL_LOCKS
*/
        box * pbc = atomic_load(&(pb->children[child->child_num]));
   ...
   */
     box * expected = atomic_load(&(Grid));
      do {
         ...
         success = TRUE;
      } while (!({; _Bool ___b = atomic_compare_exchange_weak(&(Grid), &(expected), b); ; ___b;}));
   }
   else {
      {; pthread_mutex_lock(&((G_Memory->lock_array)[(pb->particle_lock_index)])); ;};
...
InsertSubtreeInPartition (long my_id, box *b)
{
   ...
   {; pthread_mutex_unlock(&((G_Memory->lock_array)[(b->particle_lock_index)])); ;};
   */
   box_type b_type = atomic_load(&(b->type));
   if (b_type == PARENT) {
      for (i = 0; i < NUM_OFFSPRING; i++) {
      /*
#ifndef WITH_NO_OPTIONAL_LOCKS
...
#endif // WITH_NO_OPTIONAL_LOCKS
*/
	 child = atomic_load(&(b->children[i]));
	 if (child == NULL)
	    child = b->shadow[i];
	 if (child != NULL)

...
//   char particle_state[RANDOM_SIZE];
   ...
   particle_array = (particle *) ({ void* mem = malloc(Total_Particles * sizeof(particle)); assert(mem); mem; });;

   Particle_List = (particle **) ({ void* mem = malloc(Total_Particles * sizeof(particle *)); assert(mem); mem; });;
...
CreateParticleList (long my_id, long length)
{
   ...
   Local[my_id].Particles = (particle **) ({ void* mem = malloc(length
						   * sizeof(particle *)); assert(mem); mem; });;

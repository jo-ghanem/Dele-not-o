...
/*
    ...
    return too_long;
*/
	return atomic_load(&(q->n_tasks)) > n_tasks_per_queue;
}
...
*/
	long expected = atomic_load(&(global->pbar_count));
	long result;
	do {
	  if( expected >= n_processors ) result = 1;
	  else result = expected + 1;
	} while(!({; _Bool ___b = atomic_compare_exchange_weak(&(global->pbar_count), &(expected), result); ; ___b;}));
...
/*
                    ...
                    {; pthread_mutex_unlock(&(global->pbar_lock)); ;};
*/
                    ({; uint64_t ___x =atomic_fetch_sub(&(global->pbar_count), 1); ; ___x;});;
                    goto retry_entry ;
                }
/*
        {; pthread_mutex_lock(&(global->pbar_lock)); ;};
        ...
        {; pthread_mutex_unlock(&(global->pbar_lock)); ;};
*/
        bar_done = !(atomic_load(&(global->pbar_count)) < n_processors);
        }
...
if (atomic_fetch_sub(&(__count__), 1) == 1) {
	__count__ = n_processors;
	atomic_store(&(__sense__), __local_sense__);
} else {
	do {} while (atomic_load(&(__sense__)) != __local_sense__);
}
;
}
...
long _process_task_wait_loop()
{
    ...
    for( i = 0 ; i < 1000 && ! finished ; i++ )
        ...
        /*
			...
			{; pthread_mutex_unlock(&(global->pbar_lock)); ;};
		*/
			if (atomic_load(&(global->pbar_count)) >= n_processors)
				finished = 1;
        }
        }
   ...
   */
{
    ...
    while( visit_count < max_visit )
    {
		...
		*/
		...
		int exit;
		long expectedNTasks = atomic_load(&(tq->n_tasks));
		Task *expectedTop = atomic_load(&(tq->top));
		do {
			exit = 0;
			...
			do {
				...
				task_found = 1;
			} while (!({; _Bool ___b = atomic_compare_exchange_weak(&(tq->top), &(expectedTop), expectedTop); ; ___b;}));
		} while (!({; _Bool ___b = atomic_compare_exchange_weak(&(tq->n_tasks), &(expectedNTasks), expectedNTasks); ; ___b;}));
		if (exit) break;
...
*/
	long expected = atomic_load(&(global->task_counter));
	long result;
	do {
		flag = 0;
		if (expected == 0)
		...
		if (expected+1 >= n_processors)
			...
			result = expected+1;
	} while (!({; _Bool ___b = atomic_compare_exchange_weak(&(global->task_counter), &(expected), result); ; ___b;}));

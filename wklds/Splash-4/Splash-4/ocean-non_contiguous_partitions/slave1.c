...
*/
	...
	procid = ({; uint64_t ___x =atomic_fetch_add(&(global->id), 1); ; ___x;});
...
/*
   ...
   */
   ({
  ; 
  double ___oldValue = atomic_load(&(*(&global->psibi)));
  double ___newValue;
  do {
    ___newValue = ___oldValue + psibipriv;
  } while (!({_Bool ___b = atomic_compare_exchange_weak((&global->psibi), &(___oldValue), ___newValue); ___b;}));
  ; 
  ___oldValue;});;

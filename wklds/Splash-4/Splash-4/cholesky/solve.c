...
int main(int argc, char *argv[])
{
  ...
  gp = (struct gpid *) ({ void* mem = malloc(sizeof(struct gpid)); assert(mem); mem; });;
  gp->pid = 0;
  Global = (struct GlobalMemory *)
    ({ void* mem = malloc(sizeof(struct GlobalMemory)); assert(mem); mem; });;
...
*/
  ...
  MyNum = ({; uint64_t ___x =atomic_fetch_add(&(gp->pid), 1); ; ___x;});
/* POSSIBLE ENHANCEMENT:  Here is where one might pin processes to
   processors to avoid migration */

  lc =(struct LocalCopies *) ({ void* mem = malloc(sizeof(struct LocalCopies)+2*PAGE_SIZE); assert(mem); mem; });
  lc->freeUpdate = NULL;
  lc->freeTask = NULL;
  lc->runtime = 0;

...
void MallocInit(long P)
{
  ...
  mem_pool = (struct MemPool *)
    ({ void* mem = malloc((mallocP+1)*sizeof(struct MemPool)); assert(mem); mem; });;
  mem_pool++;
...
void InitOneFreeList(long p)
{
  ...
  if (p > 0) {
    mem_pool[p].freeBlock = (long **)
      ({ void* mem = malloc((MAXFAST+1)*sizeof(long *)); assert(mem); mem; });;
...
char *MyMalloc(long size, long home)
{
  ...
  if (bucket < MAXFAST) {/*
    ...
    do {
      if (!result) break;
    } while (!({; _Bool ___b = atomic_compare_exchange_weak(&(mem_pool[home].freeBlock[bucket]), &(result), NEXTFREE(result)); ; ___b;}));
  }
  ...
  if (result) {
    ...
    freespace = (long *) ({ void* mem = malloc(block_size+2*sizeof(long)); assert(mem); mem; });;
